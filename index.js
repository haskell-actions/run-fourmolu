  ...
  try {
    ...
    if (process.platform === 'linux') {
        ...
        if (useZip) {
            ...
            fourmolu_binary = path.join(extractedDir, `fourmolu-${fourmolu_version}-linux-x86_64`, 'fourmolu');
        } else {
            fourmolu_binary = path.join(path.dirname(downloaded), 'fourmolu');
            fs.renameSync(downloaded, fourmolu_binary);
        }
    }
